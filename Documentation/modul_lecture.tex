...
Darstellung von Mengen, verwendet werden.Eine sehr wichtige Funktion ist die \texttt{put}-, 
beziehungsweise die \texttt{\_put}-Funktion. Diese sorgt dafür, dass keine Duplikate in ein 
Set eingetragen werden können. Die \texttt{put}-Funktion ruft nur die \texttt{\_put}-Funktion 
auf und ist somit keine neue Methode. In Tabelle~\ref{tab:set.functions} sind die restlichen 
Mengen-Funktionen aufgelistet.
...
Bis auf syntaktische Unterschiede, wie beispielsweise der Aufruf von \texttt{arb} in Zeile 5 beider Umsetzungen, 
gibt es keine Unterschiede. Die Methoden \texttt{\_reduce}, \texttt{select\_literal}, \texttt{is\_positive} und 
\texttt{negate\_literal} sind ebenfalls nahezu identisch. Die enthaltenen Unterschiede, wie die Match-Syntax, 
wurden bereits bei vorherigen Code-Schnipseln gezeigt und somit an dieser Stelle nicht wiederholt durchleuchtet. 
Die Methode \texttt{\_reduce} hat einen Unterstrich als erstes Zeichen, das in es in Python bereits eine 
Funktion mit der Bezeichnung \texttt{reduce} gibt.
...
Allerdings wird zur Ermittlung der Summe einer Menge in \textsc{SetlX} der Operator \texttt{+/}, 
während in Python die Methode \texttt{sum} verwendet.
...
Prüfungen mit einem Profiler ergaben, dass die meiste Zeit an Aufrufen von \texttt{davis\_putnam} verloren geht. 
Es wird vermutet, dass die Sortierung der Mengen zudem eine Rolle bei der Effizienz spielen .
