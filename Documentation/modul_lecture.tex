...
nicht mehr geändert werden können, da die Mengen nur so ihren Hashwert beibehalten. Die Unveränderbarkeit 
...
Das liegt daran, dass eine wichtige Anforderung, die Erhaltung der Eleganz nicht erfüllt werden konnte. 
...
wird diese Eigenschaft hinterlegt und kann auch dort eingesehen werden. Sodass bei Mengen die interne 
...
möglichst nahe der \textsc{SetlX} Syntax zu bleiben. Die Ähnlichkeit zu \textsc{SetlX} soll 
...
Datei \textit{match.py} als Klasse Match zu finden.
...
Die Methode \texttt{createValuation} gibt eine Menge mit Tupeln, die für jedes Element in \texttt{v} 
angeben, ob es sich auch in \texttt{m} befindet, zurück. In \texttt{create\_valuation} wird 
...
Diese beiden Aussagen sind logischerweise dieselben. 
...
Sortierung der Mengen abhängt. Bei den \textsc{SetlX} Mengen liegt eine andere Sortierung vor als 
...
in ca. 500-600 Millisekunden durchläuft, werden beim Python-Skript Zeiten von 1,7-6 Sekunden gemessen. 
