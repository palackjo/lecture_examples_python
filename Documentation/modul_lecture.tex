...
Die Mengen in Python dürfen nur gewisse Werte enthalten, diese müssen unveränderbar sein. 
Zusätzlich werden auch gewisse Funktionen von den Python-Mengen nicht unterstützt, 
...
Mengen zu hinterlegen, da diese verändert werden könnten und somit der alte Hashwert ungültig werden würde. 
Diese Funktionalität wird jedoch in den \textsc{SetlX} Programmen der 
Vorlesungen mehrfach verwendet. Während bei Python-Skripten, die genauso implementiert sind, der in 
Abbildung~\ref{fig:set.error} gezeigte Fehler zurückgegeben wird. Zwar könnten bereits vorhandene sets in frozensets 
umgewandelt werden um Mengen in Mengen zu ermöglichen. Allerdings würden auch hier die umgewandelten Mengen 
nicht mehr geändert werden können, da die Mengen nur so ihren Hashwert beibehalten. %TODO: <- diesen Satz müsste man umformulieren, weil man Setl-Mengen auch nicht verändern kann. Könntest du @Johann dich darum kümmern? 
Die Unveränderbarkeit 
der Mengenelemente ist von Python dadurch bedingt, dass eine immer wiederkehrende Hashberechnung 
vermieden werden möchte. Eine Änderung wird durch die Art der Abspeicherung bereits verhindert. 
...
Eine weitere Möglichkeit, das Problem der Mengen zu lösen, wäre die Verwendung von Listen, 
anstelle von Mengen. Prinzipiell ist das in einigen Python-Übersetzungen der 
\textsc{SetlX}-Programme möglich und wurde so auch teilweise umgesetzt (siehe beispielsweise 
\textit{prime\_slim.py}). Es werden andere Datentypen verwendet, um die Informationen zu hinterlegen, 
meist Listen statt Mengen, da für die Ausführung einiger \textsc{SetlX}-Programme keine besonderen 
Eigenschaften der Mengen benötigt werden. Ein großes Problem an dieser Lösung ist allerdings, 
dass Listen nun mal keine Mengen sind und sobald Mengeneigenschaften oder Mengenoperatoren, 
die nicht für Listen gelten, verwendet werden, Listen eher ungeeignet sind. Der Workaround, 
besondere Funktionen für die Listen zu schreiben, um das Verhalten von Mengen zu imitieren, 
wurde auch als Ansatz bedacht, allerdings nach einigen kleinen Beispielübersetzungen wieder verworfen. 
Das liegt daran, dass wichtige Anforderungen, wie die Erhaltung der Eleganz, aber auch eine geringe Komplexität 
und dementsprechend auch eine angemessene Effizient nicht erfüllt werden konnte. 
Somit war dies keine Lösung, die global für alle Programme verwendet werden konnte. 
Ein weiteres Problem, das bei der Verwendung auftreten würde, ist die schlechte Skalierbarkeit 
dieser Umsetzung. Wenn größere Datenmengen auftreten, steigt die Laufzeit der Lösung mit den 
Hilfsfunktionen drastisch. Dies liegt daran, dass standardmäßig Listen in Python nicht sortiert sind 
und somit keine effizienten Algorithmen für Such- und Einfüge-Operationen gegeben sind.
Beispiele für die benötigten Funktionen, die implementiert werden mussten, um Mengeneigenschaften 
zu imitieren, waren das Entfernen von Duplikaten aus einer Liste, so wie die Ermittlung 
der Differenz zweier Listen und die Ermittlung der Potenzmenge.
...
Allerdings sind Mengen nicht der einzige Grund, warum das Modul lecture benötigt wird. 
...
Generierung einer invertierten Reihenfolge in einem String oder das Erstellen einer sortierten Liste aus einer Menge 
verwendet.~\cite{Stroetmann.30.07.2015}
...
\end{quotation}~\cite{Stroetmann.30.07.2015}
...
können, wurde eine eigene Mengen Klasse implementiert, die alle notwendigen Funktionalitäten erfüllt.

Zur internen Sicherung von Elementen innerhalb der Mengen wird der Datentyp SortedListWithKey aus dem 
Modul sortedcontainers verwendet.Das Besondere an SortedListWithKey ist, dass es sich nicht nur 
um sortierte Listen handelt, sondern die Möglichkeit besteht festzulegen, nach welcher 
Eigenschaft die Listen-Objekte sortiert werden sollen. In dem Schlüssel einer SortedListWithKey 
wird diese Eigenschaft hinterlegt und kann auch dort eingesehen werden. 
%TODO: @Johann Ich weiß nicht genau was du mit folgendem Satz sagen wolltest. Könntest du den bitte etwas umformulieren?
Sodass bei Mengen die interne 
SortedListWithKey zurückgegeben wird, da Mengen sich nicht sortieren lassen, aufgrund der 
Operator-Überladungen zur Anpassung an die \textsc{SetlX} Syntax. 

Vor der Umsetzung mit den sortierten Listen wurde das Python-package blist verwendet. 
In der alten Version der Mengen waren alle notwendigen Operationen auch möglich und es konnten 
alle Problemstellungen gelöst werden.  Jedoch war die Performance des blist Packages nicht ausreichend. 
Trotz vielversprechender Angaben zur Komplexität für Such- und Einfüge-Operationen war das Package 
deutlich langsamer als das sortedlist Package. Da bisher lediglich auf Mengen mit weniger als einer 
halben Million Elemente operiert wurde, besteht bei dem sortedlist Package für größere Datenmengen die 
Möglichkeit das diese instabil werden. Denn das sortedlist Package unterteilt die Elemente in mehrere 
Sublisten. Es wird dabei eine Liste erstellt die 100 weitere Listen enthält. Werden die Sublisten zu lang, 
verliert die Implementierung ggf. Ihren Vorteil gegenüber dem blist Package. Jedoch ist kein derartiger 
Anwendungsfall in den Vorlesungsskripten enthalten gewesen.

In der Implementierung der Mengen werden für viele Operationen Operatoren verwendet, wobei versucht wurde, 
möglichst nahe an der \textsc{SetlX} Syntax zu bleiben. Die Ähnlichkeit zu \textsc{SetlX} soll 
vorhanden sein, da diese bereits sehr mathematisch ist und somit für die Studenten sinnvoller zu lernen ist. 
...
\centering
	...
	\hline
	\textbf{\texttt{<}} & Gibt True zurück wenn die linke Menge Teilmenge aber ungleich der rechten ist \\
	\hline
	\textbf{\texttt{>}} & Gibt True zurück wenn die rechte Menge Teilmenge aber ungleich der linken ist \\
	\hline
...
Darstellung von Mengen, verwendet werden.Eine sehr wichtige Funktion ist die \texttt{put}-Funktion. 
Diese sorgt dafür, dass keine Duplikate in ein Set eingetragen werden können. 
In Tabelle~\ref{tab:set.functions} sind die restlichen Mengen-Funktionen aufgelistet.
...
\centering
	...
	\hline
	\textbf{\texttt{arb}} & Gibt ein beliebiges Element der Menge zurück \\
	\hline
	\textbf{\texttt{random}} & Gibt ein zufälliges Element der Menge zurück \\
	\hline
	\textbf{\texttt{put}} & Fügt ein Element in die Menge ein \\
	\hline
...
An dieser Stelle muss noch bemerkt werden, dass die Selektion des i-ten Elements einer Menge auch das 
Array Slicing (z.B. [1:2]), wie es von Python bekannt ist, unterstützt. Die Auswahl eines beliebigen 
Elements über den Befehl \texttt{arb()} gibt bei einer geraden Länge der Menge das letzte Element und 
bei einer ungeraden Menge das erste Element zurück. In einer früheren Implementierung wurde 
an dieser Stelle nur das erste Element zurückgegeben, jedoch traten dabei Performance Probleme in einem 
Python-Skript auf. Die Problematik wird im späteren Verlauf (Kapitel ~\ref{queens}) aufgezeigt.
...
Datei \textit{parser.py} als Klasse Match zu finden.

Der Parser erkennt gewisse Operatoren, Funktionen und Klammerungen. Die unterstützten Operatoren sind: 

„\texttt{+}“, „\texttt{-}“, „\texttt{*}“, „\texttt{/}“, „\texttt{\%}“, „\texttt{**}“, „\texttt{\&\&}“, 
„\texttt{||}“, „\texttt{<}“, „\texttt{>}“, „\texttt{<=}“, „\texttt{>=}“, „\texttt{=>}“, „\texttt{<==>}“, 
„\texttt{==}“, „\texttt{!=}“ und „\texttt{!}“.

 Die unterstützten Funktionen sind: 

„\texttt{sin}“, „\texttt{log}“, „\texttt{exp}“, „\texttt{cos}“, „\texttt{tan}“, „\texttt{asin}“, 
„\texttt{acos}“, „\texttt{atan}“, „\texttt{sqrt}“ und „\texttt{ln}“. 

Es werden runde öffnende Klammern „\texttt{(}“ und runde schließende Klammern „\texttt{)}“ erkannt.

Die wichtigste Funktion für den Benutzer ist \texttt{match(self, scheme, value)}. 
Da die Funktion auf einem erzeugten Match ausgeführt wird, sind nur die Variablen \texttt{scheme} 
...
gematched werden soll. Wichtig hierbei ist, dass das Matching nur auf Strings basierend ausgeführt werden kann, 
während Matches in \textsc{SetlX} auch die Verwendung von Literals und direkte Operationen auf den 
Ausgaben ermöglicht. Dieser Unterschied ist bei einem direkten Vergleich im Code sofort erkennbar. 
...
so werden nur die Werte aus dem Match herausgelesen und eingesetzt und nicht zusätzlich evaluiert.

%TODO: @Johann an dieser Stelle meint Herr Stroetmann, dass die Beschreibung von Match unvollständig ist. Kannst du mal drübersehen?
...
\label{puzzle}
Das Schiebepuzzle\footnote{\url{https://en.wikipedia.org/wiki/Sliding_puzzle}} ist eine Aufgabe, die den Studenten mit Lücken als Aufgabe gegeben wird, 
um Vorlesungsinhalte direkt anwenden zu können. Das bedeutet, dass der Großteil des Codes bereits gegeben ist, 
allerdings sind einige Stellen, die mit Lösungen aus der Vorlesung lösbar sind, noch frei gelassen, damit die 
Studenten die gelernte Theorie anwenden können. Mit diesem Programm sollen die Studenten eine für 
...
\begin{Verbatim}[ frame         = lines, 
    ...
    findPath := procedure(start, goal, nextStates) {
        ...
        states   := { start };
        explored := {};
    //ALT    while (states != explored) {
		    while (#states != countStates) { //NEU
				    countStates := #states; //NEU
...
während die Vorlage zwei Mengen vergleicht. Im Code-Beispiel von Abbildung \ref{fig:findpath.puzzle.setlx} 
wurde der Code so angepasst, dass in beiden Sprachen das gleiche Vorgehen gewählt wird. 
Abgesehen davon ist der Ablauf der Vorlage und der Python-Implementierung sehr ähnlich. 
...
Die Methode \texttt{move\_dir} erhält dieselben Parameter wie in der Vorlage und gibt den nächsten 
Zustand zurück, nachdem vom derzeitigen Zustand aus eine Bewegung in entweder die x- oder die 
y-Achse erfolgt. Einer der Unterschiede zur ursprünglichen Umsetzung ist hier, wie zuvor bereits 
erwähnt wurde, dass die Richtung als Tupel und nicht als Liste gewertet wird. 
(Siehe Abbildung~\ref{fig:movedir.puzzle.python} Zeile 2)

Ein erheblicher Unterschied zwischen den beiden Implementierungen ist das initiale Setzen der Variable 
\texttt{next\_state}. Während in \textsc{SetlX} der Parameter \texttt{state} dafür verwendet wird und 
eine einfache Zuweisung erfolgt, muss in Python die \texttt{list()} Funktion auf alle Listen in 
\texttt{state} verwendet werden (Abbildung~\ref{fig:movedir.puzzle.python} Zeile 3). Grund dafür ist, 
dass ansonsten die Referenzen übergeben werden und somit dann die Werte von \texttt{state} ebenfalls 
geändert werden, wenn sie in \texttt{next\_state} bearbeitet werden. Durch die \texttt{list()}-Funktion 
wird eine Kopie erzeugt und somit sind \texttt{next\_state} und state zwei 
unabhängige Listen. \textsc{SetlX} erkennt an dieser Stelle intern, ob ein Objekt geändert wurde und erstellt 
eine Kopie falls nötig. Deshalb wird dieser Aufruf nur in der Python-Version benötigt. 
...
Damit in \texttt{next\_states} eine Zahl „bewegt“ werden kann, muss über \texttt{find\_blank} das 
freie Feld gefunden werden. In der Vorlage wird in \texttt{findBlank} eine Menge erzeugt, 
die alle Reihen-Zeilen-Kombinationen enthält und aus dieser einer der Werte, an denen der übergebene 
Zustand die 0 enthält, zurückgegeben. In dieser Arbeit wird nur ein Tupel mit einem Reihen- 
und einem Zeilenwert, an denen der Zustand die Null enthält zurückgegeben. Zu Vergleichszwecken ist 
die Funktion, so wie sie in Python geschrieben wurde, auch in \textsc{SetlX} implementiert. 
In \textsc{SetlX} merkt man keine Unterschiede in der Performance. Diese Methode zeigt wie ähnlich der Code in 
\textsc{SetlX} und Python sein kann.
...
sein. Die Operatoren der Sets des Python-Moduls „lecture“ wurden basierend auf die Implementierung, wie sie in \textsc{SetlX} 
verwendet werden, umgesetzt.

\subsection{Watson}
%TODO: @Johann Herr Stroetmann meint, dass folgender Absatz nur Geschwafel ist. Wenn du der selben Meinung bist, kannst du ihn löschen
...
Bei der Aufgabe Watson sind einige Tatsachen zu einem Mordfall bekannt. 
Anhand dieser gegebenen Aussagen sollen die Täter aus drei Verdächtigen herausgefunden werden. 
Das heißt es muss die Menge an aussagenlogischen Formeln ausgewertet werden. Als Resultat soll eine 
Menge von Tätern, die in keinem Widerspruch zu den Aussagen steht, gegeben sein.
...
und „False“ in der Match-Implementierung nicht eingetragen sind. Außerdem hat das Herauslassen dieser beiden 
Fälle keine Auswirkungen auf die Ausführung des Skripts. Wenn der Ausdruck \texttt{g => h} 
...
Die Methode \texttt{createValuation} gibt eine Menge mit Paaren, die für jedes Element in \texttt{v} 
angeben, ob es sich auch in \texttt{m} befindet, zurück. Demnach erhält jedes Element aus \texttt{v} 
in der neuen Liste einen Boolean, der die Beziehung zu \texttt{m} beschreibt. In \texttt{create\_valuation} wird 
das gleiche Resultat geliefert, allerdings befinden sich die Tupel in einer Liste. 
Die Struktur wurde so gewählt, da zu einem späteren Verlauf aus einer solchen Menge/Liste der 
Wahrheitswert \texttt{(x in m)} durch das Aufrufen aus der Liste, anhand der Bezeichnung des 
Elements abgefragt wird. In Python ist das mit einem Dictionary möglich, jedoch unterstützt 
Python eine Sortierung von Dictionaries nicht, weshalb diese nicht in die Sets eingefügt werden können. 
Das heißt dass keine Dictionaries in Sets möglich sind. Die Struktur, Key-Value-Paare (zweistellige Tupel) 
in einer Liste zu hinterlegen, kann allerdings über den \texttt{dict()}-Befehl in ein Dictionary 
konvertiert werden. Somit wird die Liste zum Abspeichern in einem Set verwendet und 
sobald die Valuation wieder ausgelesen wird, wird diese als Dictionary weiterverwendet.

Nun werden die Tatsachen, die zu dem Mordfall bekannt sind, als Strings von Bedingungen eingelesen. 
In \textsc{SetlX} werden diese über den parse-Befehl in Variablen gelesen, während in Python die 
Strings direkt in die Variablen \texttt{f1} bis \texttt{f6} gespeichert werden.
...
\texttt{all} verwendet. Beide Funktionen geben nur dann \texttt{True} zurück, wenn alle Elemente 
der angegebenen Liste/Menge ebenfalls den Wahrheitswert \texttt{True} besitzen. In Python werden die 
erzeugten Tupel in Dictionaries gespeichert, um den Aufruf für \texttt{evaluate} und die Ausgabe zu 
erleichtern. Zuletzt wird das Ergebnis noch ausgegeben.
...
Pro Schleifendurchgang werden die nächst möglichen Pfadschritte berechnet und geprüft, 
...
ob die Menge \texttt{found} "\texttt{!=}" eine leere Menge ist (Abbildung~\ref{fig:findpath.wgc.setlx} Zeile 7), 
wird in Python geprüft ob \texttt{found} "\texttt{not}" eine leere Menge ist (Abbildung~\ref{fig:findpath.wgc.python} Zeile 7). 
Diese beiden Aussagen sind logischerweise dieselben, sie unterscheiden sich nur in der Schreibweise. 
...
Die Funktion \texttt{no\_cycle} prüft, ob es zu Zyklen kommen kann, indem geprüft wird wie viele 
gleiche Elemente in \texttt{l1} und \texttt{l2} enthalten sind. Nur wenn es nur ein Element, 
das in beiden Listen enthalten ist, gibt, tritt kein Zyklus auf.
...
mit \texttt{find\_path()} ermittelt werden, indem als Start \texttt{wgc\_all}, als Ziel eine leere Menge 
und als mögliche Pfade \texttt{r} übergeben werden.

Im Allgemeinen kann gesagt werden, dass dieses Programm mit Hilfe der Set-Implementierung sehr gut in 
Python nachkonstruiert werden konnte. Im Python-Skript sind nur wenige Abweichungen auffindbar. 

\subsection{8 Damen Problem}
\label{queens}
Das 8 Damen Problem beschäftigt sich mit der Frage, wie acht Damen auf ein Schachbrett 
gestellt werden können, ohne sich gegenseitig schlagen zu können. Als Hinführung zu diesem 
Problem wird den Studenten zuvor das Davis-Putnam-Verfahren vorgestellt. Nachdem das 
Davis-Putnam-Verfahren implementiert wurde, soll anhand dessen das 8 Damen Problem gelöst werden.
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{\texttt{davisPutnam} in Davis Putnam (\textsc{SetlX})}
\label{fig:davisputnam.dp.setlx}
\end{figure}
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{\texttt{davis\_putnam} in Davis Putnam (Python)}
\label{fig:davisputnam.dp.python}
\end{figure}

Zu Beginn der \texttt{davis\_putnam}-Funktion wird die \texttt{saturate}-Methode auf die Menge \texttt{clauses} ausgeführt. 
Das Ergebnis wird in die Variable \texttt{s} gespeichert, während in \textsc{SetlX} dafür die Variable 
\texttt{clauses} überschrieben wird. Daraufhin wird geprüft, ob die ermittelte Menge eine leere Menge enthält, 
da in diesem Fall das Ergebnis ein Falsum wäre. Dadurch würde eine leere Menge in einer leeren Menge zurückgegeben werden. 
Wenn in \texttt{s} nur Unit-Klauseln enthalten sind, so ist das Ergebnis auch bereits ermittelt und es wird \texttt{s} 
zurückgegeben. Auch wenn beide if-Abfragen leicht unterschiedlich aussehen führen sie jedoch die gleiche Abfrage aus. 
Danach wird ein beliebiges Literal aus \texttt{s}, das sich nicht in \texttt{literals} befindet gewählt 
und in einer weiteren Variable negiert. Nun folgt ein rekursiver Aufruf, bei dem \texttt{s} mit \texttt{l} addiert 
und \texttt{literals} mit sowohl \texttt{l} als auch \texttt{not\_l} addiert, als Parameter übergeben werden. 
Wenn das Ergebnis keine Menge einer leeren Menge ist, wird es zurückgegeben. Anderen Falls wird das Ergebnis des 
rekursiven Aufrufs mit \texttt{not\_l} anstelle von \texttt{l} zurückgegeben.

Zwischen den beiden Implementierungen ist kein bedeutender Unterschied zu bemerken. Das Einzige, 
das bei der Übersetzung bedacht werden muss ist, dass Funktionen wie \texttt{len} oder \texttt{all} 
bzw. einzeilige for-Schleifen in beiden Sprachen unterschiedlich dargestellt werden.
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{\texttt{saturate} in Davis Putnam (\textsc{SetlX})}
\label{fig:saturate.dp.setlx}
\end{figure}
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{\texttt{saturate} in Davis Putnam (Python)}
\label{fig:saturate.dp.python}
\end{figure}

In \texttt{saturate} werden alle Klauseln, die mit Unit-Schnitten aus \texttt{s} ableitbar sind zurückgegeben. 
Hierbei werden die Klauseln, auf denen ein Unit-Schnitt bereits durchgeführt wurde, aus \texttt{s} entfernt.

Hier ist auch wieder zu erkennen wie groß die Ähnlichkeit der beiden Implementierungen ist. 
Bis auf syntaktische Unterschiede, wie beispielsweise der Aufruf von \texttt{arb} in Zeile 5 
beider Umsetzungen, gibt es keine Unterschiede. Die Methoden \texttt{\_reduce}, 
\texttt{select\_literal}, \texttt{is\_positive} und \texttt{negate\_literal} sind ebenfalls nahezu identisch. 
Die enthaltenen Unterschiede, wie die Match-Syntax, wurden bereits bei vorherigen Code-Schnipseln gezeigt 
und somit an dieser Stelle nicht wiederholt durchleuchtet. Die Methode \texttt{\_reduce} hat einen 
Unterstrich als erstes Zeichen, da es in Python bereits eine Funktion mit der Bezeichnung 
\texttt{reduce} gibt.
Bei der Implementierung des Skriptes ist aufgefallen, dass der Algorithmus stark von der 
Sortierung der Mengen abhängt. %TODO: @Johann kannst du bitte den folgenden Satz erklären, also warum eine andere Sortierung vorliegt etc.
Bei den \textsc{SetlX} Mengen liegt eine andere Sortierung vor als 
bei den Python Mengen, zufälligerweise war die Sortierung der \textsc{SetlX} besser auf den 
Algorithmus ausgelegt. Das führte dazu, dass die Berechnung der \textit{queens.stlx} einige 
Sekunden in Anspruch nahm, während hingegen in Python eine halbe Stunde gewartet werden musste. 
Um die Sortierung der Python Mengen anzupassen wurden bei der Auswahl der Literale lediglich positive 
Literale zugelassen und der \texttt{arb()}-Befehl an den von \textsc{SetlX} angepasst, sodass in der 
\texttt{saturate()} Funktion nun nicht mehr ausschließlich das erste Element gewählt wurde. 
Dies führte, auch mit Python Mengen, innerhalb einiger Sekunden zu einem Ergebnis. 
Es wurde ebenfalls in \textsc{SetlX} der Einsatz von \texttt{first()} getestet, 
dieser ergab jedoch keine Nennenswerten Unterschiede in der Dauer der Berechnung, 
weshalb die Sortierung der Mengen selbst ausschlaggebend zu sein scheint. Eine grundsätzliche 
Verwendung von Random in der \texttt{saturate()} Funktion wurden ebenfalls in Erwägung gezogen, 
jedoch führte diese bereits in einigen Testfällen zu mehreren ausschlagenden Werten mit einer 
Verschlechterung um einen Faktor von Zehn. Somit ist für den weiteren Einsatz der Sets immer die 
Stabilität von Algorithmen zu beachten sowie eine Betrachtung der Sortierung nötig.

Da damit der Davis-Putnam-Algorithmus implementiert ist, kann dieser nun in Queens (8 Damen Problem) 
verwendet werden.
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{\texttt{atMostOne} in Davis Putnam (\textsc{SetlX})}
\label{fig:atmostone.dp.setlx}
\end{figure}
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{\texttt{at\_most\_one} in Davis Putnam (Python)}
\label{fig:atmostone.dp.python}
\end{figure}

Bei der Betrachtung der Methode \texttt{at\_most\_one} fällt auf, dass in Python Strings verwendet werden, während in 
\textsc{SetlX} eine einfache Negierung stattfindet. Dies liegt daran, dass in \textsc{SetlX} für die Feldpositionen 
Variablen erzeugt werden, was jedoch in Python nicht möglich ist. Die Lösung mit den Strings erschwert zwar 
teilweise die Schreibweise, erfüllt jedoch denselben Zweck wie die Lösung in \textsc{SetlX}. 
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{\texttt{atMostOneInRow} in Davis Putnam (\textsc{SetlX})}
\label{fig:amoir.dp.setlx}
\end{figure}
\begin{figure}[!ht]
\centering
\begin{Verbatim}[ frame         = lines, 
    ...
    def at_most_one_in_row(row, n):
        return at_most_one(Set("varr%sc%s" % (str(row), str(column))
                           for column in range(n)))
\end{Verbatim}
\vspace*{-0.3cm}
\caption{\texttt{at\_most\_one\_in\_row} in Davis Putnam (Python)}
\label{fig:amoir.dp.python}
\end{figure}

In der Funktion \texttt{at\_most\_one\_in\_row} kann gesehen werden, wie die „Variablen“ in Python dargestellt werden. 
Das Präfix „var“ steht für Variable, nach dem „r“ kommt die Reihennummer (\texttt{row}) und nach dem „c“ die 
Zeilennummer (\texttt{column}). An dieser Stelle wird wieder, wie bereits zuvor, die automatische Generierung einer 
Liste in \textsc{SetlX} über den \texttt{range}-Befehl in Python gelöst.

Die Funktion \texttt{one\_in\_column} ist sehr ähnlich zu \texttt{at\_most\_one\_in\_row} und gleich aufgebaut. 
Deshalb wird die Methode an dieser Stelle nicht dargestellt. 
Anstatt die Rückgabe von \texttt{at\_most\_one} zurückzugeben, wird hier eine Menge zurückgegeben. 
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{\texttt{atMostOneInLowerDiagonal} in Davis Putnam (\textsc{SetlX})}
\label{fig:amoild.dp.setlx}
\end{figure}
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{\texttt{at\_most\_one\_in\_lower\_diagonal} in Davis Putnam (Python)}
\label{fig:amoild.dp.python}
\end{figure}

In \texttt{at\_most\_one\_in\_lower\_diagonal} lässt sich wieder erkennen, dass, bis auf die Eigenart des 
Strings als Variable, beide Implementierungen derselben Struktur folgen. 
Die Implementierungen sind sehr gut gegenüberstellbar.

In \texttt{at\_most\_one\_in\_upper\_diagonal} ist die Umsetzung analog zu der in \texttt{at\_most\_one\_in\_lower\_diagonal}.
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{\texttt{allClauses} in Davis Putnam (\textsc{SetlX})}
\label{fig:allclauses.dp.setlx}
\end{figure}
\begin{figure}[!ht]
\centering
\begin{Verbatim}[ frame         = lines, 
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{\texttt{all\_clauses} in Davis Putnam (Python)}
\label{fig:allclauses.dp.python}
\end{figure}

Beide Ausführungen von \texttt{all\_clauses} weisen wieder große Ähnlichkeiten auf. 
Allerdings wird zur Ermittlung der Summe einer Menge in \textsc{SetlX} der Operator "`\texttt{+/}"' verwendet, 
während in Python die Methode \texttt{sum} eingesetzt wird.
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{\texttt{solve} in Davis Putnam (\textsc{SetlX})}
\label{fig:solve.dp.setlx}
\end{figure}
\begin{figure}[!ht]
\centering
\begin{Verbatim}[ frame         = lines, 
...
\end{Verbatim}
\vspace*{-0.3cm}
\caption{\texttt{solve} in Davis Putnam (Python)}
\label{fig:solve.dp.python}
\end{figure}

Da nun alle benötigten Methoden definiert wurden, kann die Lösung ermittelt werden. Zuerst werden alle Klauseln 
mit dem \texttt{all\_clauses}-Befehl erstellt. Diese werden daraufhin ausgegeben und dann das Problem anhand von 
\texttt{davis\_putnam} gelöst. Zuletzt wird, sofern ein Ergebnis vorhanden ist, dieses Ausgegeben, 
oder eine Fehlermeldung ausgegeben.

Die Zeit für \textit{queens.py} wird, wie beim Schiebepuzzle, gemessen. Die nachfolgenden Messwerte wurden mit 
demselben Rechner, wie er in ~\ref{puzzle} beschreiben wurde, ermittelt. Während das \textsc{SetlX}-Programm 
in ca. 500-600 Millisekunden durchläuft, werden beim Python-Skript Zeiten von 1,7-6 Sekunden gemessen. %TODO: @Johann kannst du bitte an dieser Stelle nochmal erwähnen wo ein Random vorhanden ist?
Prüfungen mit einem Profiler ergaben, dass vergleichbar viele Aufrufe von Methoden in beiden 
Sprachen getätigt werden.Hier scheint erneut die Java Virtual Machine, welche in \textsc{SetlX} genutzt wird,  
der Python Virtual Machine weit überlegen.
